...
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Error "Administrator privileges are required. Please re-launch PowerShell as an Administrator."
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
            ...
            <Setter Property="Template">
                <Setter.Value>
                    <ControlTemplate TargetType="RadioButton">
                        <Border Background="{TemplateBinding Background}" Padding="{TemplateBinding Padding}" BorderBrush="Transparent" BorderThickness="0,0,0,3">
    ...
    <Grid>
        <Grid.RowDefinitions>
            <RowDefinition Height="Auto"/> <!-- Main Navigation Bar -->
        ...
        <Grid Grid.Row="1" x:Name="ContentArea">
            ...
            <Grid x:Name="ViewInfo" Visibility="Collapsed">
                 <ScrollViewer VerticalScrollBarVisibility="Auto" Margin="5"><StackPanel x:Name="SystemInfoPanel" Margin="20"/></ScrollViewer>
            </Grid>
        ...
        <!-- Status Bar -->
        <StatusBar Grid.Row="2" Background="{StaticResource AccentColor}" Foreground="White">
            <StatusBarItem><TextBlock x:Name="StatusText" Text="Ready"/></StatusBarItem>
            <StatusBarItem HorizontalAlignment="Right"><TextBlock x:Name="AdminStatusText" Text="Admin Privileges: Yes"/></StatusBarItem>
        </StatusBar>
        
        <!-- Progress Overlay -->
        <Grid x:Name="ProgressOverlay" Grid.RowSpan="3" Background="#80000000" Visibility="Collapsed">
            <Border HorizontalAlignment="Center" VerticalAlignment="Center" Background="{StaticResource PrimaryBgColor}" CornerRadius="5" Padding="30"><StackPanel><ProgressBar IsIndeterminate="True" Width="300" Height="20"/><TextBlock x:Name="ProgressText" Text="Working..." Foreground="White" FontSize="16" HorizontalAlignment="Center" Margin="0,15,0,0"/></StackPanel></Border>
        </Grid>
    </Grid>
</Window>
"@
    ...
    $controls = @{}; $xaml.SelectNodes("//*[@*[contains(translate(name(.),'n','N'),'x:Name')]]") | ForEach-Object { $controls[$_.Name] = $window.FindName($_.Name) }

    $script:updateCollection = New-Object System.Collections.ObjectModel.ObservableCollection[PSCustomObject]; $controls.UpdateDataGrid.ItemsSource = $script:updateCollection
    $script:allDriversCollection = New-Object System.Collections.ObjectModel.ObservableCollection[PSCustomObject]; $script:driversView = [System.Windows.Data.CollectionViewSource]::GetDefaultView($script:allDriversCollection); $controls.DriverManagerDataGrid.ItemsSource = $script:driversView
    #endregion

    #region Helper Functions & Navigation
    function Update-Status { param([string]$Message) $window.Dispatcher.Invoke({ $controls.StatusText.Text = $Message }) }
    function Set-ProgressState { param([bool]$IsActive, [string]$Message = "Working...") $window.Dispatcher.Invoke({ $controls.ProgressText.Text = $Message; $controls.ProgressOverlay.Visibility = if ($IsActive) { 'Visible' } else { 'Collapsed' } }) }
    ...
    foreach ($navButton in $navButtons) {
        $navButton.Value.add_Checked({
            param($sender, $e)
            $viewName = "View" + $sender.Name.Replace("Nav","")
            $controls.GetEnumerator() | Where-Object { $_.Key -like "View*" } | ForEach-Object {
                $_.Value.Visibility = if ($_.Key -eq $viewName) { 'Visible' } else { 'Collapsed' }
            }
        })
    }
    #endregion
    ...
    # --- Update Center ---
    $controls.ScanButton.add_Click({ if (-not (Ensure-PSWindowsUpdate)) { return }; Set-ProgressState -IsActive $true -Message "Scanning..."; Update-Status "Scanning..."; $script:updateCollection.Clear(); $controls.InstallButton.IsEnabled = $false; $script:currentJob = Start-Job -ScriptBlock { Import-Module PSWindowsUpdate; Get-WindowsUpdate -CategoryIDs '0fa1201d-4330-4fa8-8ae9-b877473b6441' -ErrorAction SilentlyContinue }; $script:currentTimer = New-Object System.Windows.Threading.DispatcherTimer; $script:currentTimer.Interval = [TimeSpan]::FromSeconds(1); $script:currentTimer.add_Tick({ if ($script:currentJob.State -ne 'Running') { $script:currentTimer.Stop(); $updates = Receive-Job $script:currentJob; Remove-Job $script:currentJob; if ($null -eq $updates -or $updates.Count -eq 0) { Update-Status "No new driver updates found." } else { $updates | ForEach-Object { $script:updateCollection.Add([PSCustomObject]@{ IsSelected = $false; Title = $_.Title; SizeMB = "{0:N2}" -f ($_.Size / 1MB); KB = $_.KB; UpdateObject = $_ }) }; Update-Status "Found $($updates.Count) driver update(s)." }; Set-ProgressState -IsActive $false } }); $script:currentTimer.Start() })
    $controls.InstallButton.add_Click({ $updatesToInstall = $script:updateCollection | Where-Object { $_.IsSelected }; if ($updatesToInstall.Count -eq 0) { return }; $confirm = [System.Windows.MessageBox]::Show($window, "This will install $($updatesToInstall.Count) driver(s). The system may reboot automatically. Proceed?", "Confirm Installation", 'YesNo', 'Warning'); if ($confirm -ne 'Yes') { Update-Status "Installation cancelled."; return }; Set-ProgressState -IsActive $true -Message "Installing..."; Update-Status "Installing..."; $originalUpdates = $updatesToInstall.UpdateObject; $script:currentJob = Start-Job -ScriptBlock { param($updates) Import-Module PSWindowsUpdate; Install-WindowsUpdate -InputObject $updates -AcceptAll -AutoReboot } -ArgumentList (,$originalUpdates); $script:currentTimer = New-Object System.Windows.Threading.DispatcherTimer; $script:currentTimer.Interval = [TimeSpan]::FromSeconds(1); $script:currentTimer.add_Tick({ if ($script:currentJob.State -ne 'Running') { $script:currentTimer.Stop(); $result = Receive-Job $script:currentJob; Remove-Job $script:currentJob; Set-ProgressState -IsActive $false; Update-Status "Installation finished."; $script:updateCollection.Clear(); $controls.InstallButton.IsEnabled = $false } }); $script:currentTimer.Start() })
    $controls.UpdateDataGrid.add_MouseUp({ Start-Sleep -Milliseconds 50; $controls.InstallButton.IsEnabled = ($script:updateCollection | Where-Object { $_.IsSelected }).Count -gt 0 })
    ...
    # --- System Info ---
    function Populate-SystemInfo { $panel = $controls.SystemInfoPanel; $panel.Children.Clear(); function Add-Heading { param($Text) $tb = New-Object System.Windows.Controls.TextBlock; $tb.Text = $Text; $tb.FontSize = 16; $tb.FontWeight = [System.Windows.FontWeights]::Bold; $tb.Foreground = $window.FindResource('TextColor'); $tb.Margin = "0,15,0,5"; $panel.Children.Add($tb) }; function Add-Item { param($Key, $Value) if (-not [string]::IsNullOrWhiteSpace($Value)) { $tb = New-Object System.Windows.Controls.TextBlock; $tb.Text = "$Key`: $Value"; $tb.Foreground = $window.FindResource('FadedTextColor'); $tb.Margin = "10,2,0,2"; $panel.Children.Add($tb) } }; Add-Heading 'Operating System'; $os = Get-CimInstance -ClassName Win32_OperatingSystem; Add-Item 'Name' $os.Caption; Add-Item 'Version' $os.Version; Add-Item 'Build' $os.BuildNumber; Add-Heading 'Processor'; $cpu = Get-CimInstance -ClassName Win32_Processor; Add-Item 'Name' $cpu.Name; Add-Item 'Cores' $cpu.NumberOfCores; Add-Item 'Threads' $cpu.NumberOfLogicalProcessors; Add-Heading 'Graphics Card'; $gpus = Get-CimInstance -ClassName Win32_VideoController; foreach ($gpu in $gpus) { Add-Item $gpu.Name "$([math]::Round($gpu.AdapterRAM / 1GB, 2)) GB" }; Add-Heading 'Memory'; $mem = Get-CimInstance -ClassName Win32_ComputerSystem; Add-Item 'Total RAM' "$([math]::Round($mem.TotalPhysicalMemory / 1GB, 2)) GB"; Add-Heading 'Motherboard'; $board = Get-CimInstance -ClassName Win32_BaseBoard; Add-Item 'Manufacturer' $board.Manufacturer; Add-Item 'Product' $board.Product; Add-Heading 'BIOS'; $bios = Get-CimInstance -ClassName Win32_BIOS; Add-Item 'Manufacturer' $bios.Manufacturer; Add-Item 'Version' $bios.SMBIOSBIOSVersion }; Populate-SystemInfo
